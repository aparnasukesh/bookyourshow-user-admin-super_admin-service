...
func InitResources(cfg config.Config) (func() error, error) {
	...
	theaterClient, err := grpcClient.NewTheaterGrpcClient(cfg.GrpcMovieBookingPort)
	if err != nil {
		return nil, err
	}
	...
	superAdminService := superadmin.NewService(superAdminRepo, notificationClient, authClient, movieBookingClient, theaterClient)
